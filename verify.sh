#!/usr/bin/env bash
# Verify the cqX on-chain program matches this repo.
...
echo "[verify] reproducibly building cqX via solana-verify (uses docker)"
solana-verify build --library-name cqX

LOCAL_HASH=$(solana-verify get-executable-hash target/deploy/cqx.so)
