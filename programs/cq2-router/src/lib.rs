//! cq2-router — on-chain dispatcher for every DEX cq2 routes through.
//!
...
solana_security_txt::security_txt! {
    ...
    source_code: "https://github.com/czlabsIO/cq2-router",
    ...
    acknowledgements: "rv2 router (czlabsIO/v2-router) ancestral codebase. neodyme-labs/solana-security-txt for the badge format."
}
...
pub mod cq2_router {
    ...
    pub fn swap<'info>(
        ...
        require!(fee_bps <= MAX_FEE_BPS, Cq2RouterErrorCode::InvalidFeeBps);
        let mut i: usize = 0;
        let mut amount = in_amount;

        require!(
            amount > 0,
            Cq2RouterErrorCode::AmountInMustBeGreaterThanZero
        );
        require!(
            slippage_bps <= 10_000,
            Cq2RouterErrorCode::InvalidSlippageBps
        );
            ...
            if temp_wsol_ai.data_is_empty() {
                ...
                let total_lamports = if wsol_is_start_input {
                    rent_lamports
                        ...
                        .ok_or(Cq2RouterErrorCode::MathOverflow)?
                } else {
                    rent_lamports
                };
        ...
        for (_step_idx, step) in route_plan.iter().enumerate() {
            ...
            require!(
                ...
                Cq2RouterErrorCode::InvalidPercentBps
            );
            ...
            let alloc_u128: u128 = if step.is_last_percent {
                ...
                let remaining_amt = original
                    ...
                    .ok_or(Cq2RouterErrorCode::MathUnderflow)?;
                if remaining_amt == 0 {
                    continue;
                }
                remaining_amt
            ...
            require!(
                ...
                Cq2RouterErrorCode::MathOverflow
            );
            ...
            let out_amt: u64 = match &step.swap {
                ...
                _ => {
                    return Err(error!(Cq2RouterErrorCode::InvalidSwapRoute));
                }
            };
        ...
        let slippage_multiplier = 10_000u64
            ...
            .ok_or(Cq2RouterErrorCode::InvalidSlippageBps)?;
        ...
        if amount < adjusted_out_amount {
            return Err(Cq2RouterErrorCode::SlippageExceedDesireLimit.into());
        }

        msg!("🌐 carbium cq2-router swap: gross_out={} legs={}", amount, route_plan.len());
        ...
        if fee_amount > 0 {
            require!(remaining.len() >= 1, Cq2RouterErrorCode::InvalidSwapRoute);
            let n = remaining.len();
            ...
            let token_program_ai = if wsol_account_used {
                ...
                require!(remaining.len() > TOKEN_PROGRAM_INDEX, Cq2RouterErrorCode::InvalidSwapRoute);
                remaining[TOKEN_PROGRAM_INDEX].clone()
            };

            let fee_source = if output_is_sol {
                // For SOL output the swap deposited into temp_wsol (managed by
                // cq2-router PDA but authority = user via initialize_account3).
                ...
                require!(n >= 2, Cq2RouterErrorCode::InvalidSwapRoute);
                remaining[n - 2].clone()
            };
