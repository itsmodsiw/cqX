...
//!   [9..17]  min_amount_out (u64 LE, = 0 — slippage enforced by cq2-router)
...
pub fn bisonfi_swap(
    ...
    require!(last.key() == *program, Cq2RouterErrorCode::ProviderNotFound);
    require!(direction <= 1, Cq2RouterErrorCode::InvalidSwapRoute);
    require!(dst_idx < remaining_accounts.len() - 1, Cq2RouterErrorCode::InvalidSwapRoute);
